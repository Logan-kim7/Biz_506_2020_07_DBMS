...
--이상현상 
--추가이상, 변경(수정)이상, 삭제이상
--DBMS에 저장한, 저장된 데이터를 관리할때(SELECT 외의 CUD에서)
--문제가 발생하여 원본데이터가 무결성을 잃는 상황


--추가이상 방지 : PK(기본키), UNIQUE, NOT NULL 이러한 제약조건을 설정하여
-- 잘못된 데이터가 INSERT 되는 현상을 막는것
-- PK로 설정된 칼럼은 정말 천재지변으로 문제가 발생하지 않는한 저장된 값은 
-- 변경, 삭제를 금해야 한다.
-- 필요없는 데이터가 발생하는 경우 데이터를 삭제하는 대신
-- 특정한 칼럼을 하나 마련하고, 그칼럼에 데이터의 사용 유무를 등록하여
-- 관리하는 것이 실무상 원칙!


--변경, 삭제 이상 방지 : 특별한 경우가 아니면 2개 이상의 Recode에
--변화가 발생하지 않도록 코드나 관리를 설계해야한다.


-- 어떤 테이블에 변경될 가능성이 1이라도 존재하는 칼럼이 있는데
-- 여기에 동일한 값들이 중복되어 저장되어 있을때
-- 이칼럼의 값은 여러 레코드가 변경, 삭제 되는 일이 생길수 있다.
-- 더불어 이러한 변경, 삭제를 수행하는 동안 데이터의 문제가 발생할 수 있다.
...
--------------------------------------------------------------------------------
--학생정보를 조회하면서 학과테이블과 연결하여 조회하는 CODE생성
--------------------------------------------------------------------------------

-- 기본 SELECT * 를 시작
-- FORM 절에 보고자하는 주 Table(Master)를 작성
-- LEFT JOIN절에 보조 정보가 담긴 Table을 작성
-- ON 두 테이블의 연관관계를 설정하는 키를 지정
-- SELECT에 필요한 칼럼들을 나열

-- JOIN 이 된 후에 보조테이블의 칼럼을 WHERE 조건설정을 하여 SELECTION을 수행할수 있다.
...
-- 이러한 상항에서 JOIN을 수행하면 칼럼을 제대로 인식하지 못하여
-- 오류가 발생하는 경우가 있다.(실제로 많다.)
-- 이런경우 table에 AS를 설정하여 주면 오류를 막을수 있다. 
--그 AS는 [table] AS [alias] 형식
--단, 오라클에서는 AS키워드를 사용하지 않는다.
SELECT ST.st_num, ST.st_name, ST.st_dept, DT.d_name, DT.d_prof, DT.d_tel, DT.st_grade, ST.st_tel,ST.st_addr
FROM tbl_student  ST
...
-- SELECT 문을 이용해서 복잡한 Query를 작성하고
-- 작성된 Query를 자주사용하게 될 것으로 예상이 되면
