...
SELECT AVG(st_grade),AVG(st_AVG) FROM tbl_student;

-- tbl_student의 st_grade 칼럼에 저장된 값중에서 
-- 최대값(가장 큰 값), 최소값(가장 작은값)을 보여달라
...
-- tbl_student 데이터중에서 학과가 어떠한 학과가 있는지 알고싶다.
-- 학과이름이 같은 이름들이 다수 존재하는데 이들 이름을 1개씩만 추출하여
--      리스트로 보여준다면 어떠한 학과가 있느지 알기 쉬울것 같다.
-- st_dept 칼럼에 저장된 값들을 같은 값끼리 그룹을 지어라
-- 그 그룹지은 이름을 1개씩만 나열해달라
...
-- st_name 칼럼에 저장된 값들이 중복된 값이 없어서 
-- GROUP BY가  의미가 없는 경우도 있다.
...
-- 각 학과의 학생들중에 
-- 나이가 가장많은 학생의 나이와
-- 나이가 가장적은 학생의 나이를 보여달라
-- 단, 가장나이가많은 학생의 이름, 가장나이가 적은 학생의 이름을 찾는데는
-- 이 SQL로는 어렵다.
...
-- SQL 명령문 : Query(쿼리 = 질의) 문 이라고도 부른다.
-- 질문 : 정답이 정해진문제
-- 질의 : 장황하게 설명이 필요한 것.