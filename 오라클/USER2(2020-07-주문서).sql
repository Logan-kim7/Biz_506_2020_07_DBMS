...
-- tbl_order Table을 만들어서 여기에 추가될 데이터들 중에 1개의 칼럼으로는 PK를 만들수 없어서
...
-- 이러한 불편을 방지하기 위해 SEQENCE 라는 객체를  사용하여, 자동으로 일련번호를 만드는 방법을  사용한다.
CREATE SEQUENCE seq_order
START WITH 1 INCREMENT BY 1 ;

-- 표준 SEQ에서 간단한 계산을 할때
-- SELECT 3 + 4; 라고 코딩을 하면 3+4의 결과를 확인 할 수 있다.
-- 그런데 오라클에서는 SELECT 명령문에 FROM[talbe] 절이 없으면 문법오류가 난다.
...
-- 주문번호, 고객번호, 상품코드 이 3개의 칼럼 묶음에 값이 중복되면 INSERT가 되지 않도록 제약조건을 설정해야한다.
...
-- duplicate keys found : PK , UNIQUE 로 설정된 칼럼에 값을 추가하거나, 
--                      이미 중복된 값이 있는데 PK, UNIQUE를 설정하려 할때 발생을 한다.
-- UNIQUE 제약조건을 추가하기 위해 마지막에 추가된 데이터를 제거하려고 다음과 같은 SQL을 실행한다.
-- 이 SQL을 실행하면, 삭제되어서는 안되는 중요한 주문정보가 1건이 같이 삭제 되어 버릴 것이다.
-- 이 Table 의 데이터는 무결성을 잃게 된다. => 삭제이상

DELETE FROM tbl_order
    WHERE o_num = 'O00001' AND o_cnum = 'C0032' AND o_pcode = 'P00001';
    
-- 다행이 일련번호 칼럼이 만들어져 있기 때문에 PK를 기준으로 값을 삭제할수 있다.
...
-- 복수의 칼럼으로 PK를 설정하는데
-- UPDATE, DELETE 수행할때  WHERE 칼럼1 = 값 AND 칼럼2 = 값 AND 칼럼3 = 값 ..... 과 같은 조건을 부여해야한다.
-- 이것은 데이터의 무결성을 유지하는데 매우 좋지 않은 환경이다.
...
INSERT INTO tbl_order(o_seq,o_date, o_num, o_cnum, o_pcode) VALUES (seq_order.NEXTVAL,'2020-07-22','O00022','C0055','P00067');
COMMIT;