...
-- 이 데이터를 DB에 저장하려고 봤더니, 이 데이터의 PK칼럼으로 지정할 만한 칼럼을 찾을 수가 없다.
-- 이러한 경우(Work DB인 경우)에는 실제 데이터와 별도로 임의의 PK 칼럼을 생성해서 
-- 관리 해주는 것이 좋다.
...
-- 검색을 해봤더니 위 3가지 상품의 거래 내역이 다른 것이 없다.
...
-- 혹시 다른 매입, 매출 데이터가 있을 경우 막대한 문제를 일으킬수 있다.
-- 반드시 PK 칼럼값을 조회하고 PK 칼럼으로 WHERE 절을 사용하자
UPDATE tbl_iolist
SET io_price = 1000,  --단가
    io_amt = io_pty * 1000 --금액(수량 * 단가)
WHERE io_seq = 5;

-- 7 + 8 칫솔
UPDATE tbl_iolist
SET io_price = 2500,  --단가
    io_amt = io_pty * 2500 --금액(수량 * 단가)
WHERE io_seq = 10;
...
-- 원본 데이터는 
--      수량, 매입단가, 매출단가, 매입금액, 매출금액, 으로 칼럼을 분리하여 사용을 해왔다.
...
--   (거래처, 수량, 매입단가, 매입금액, 매출단가, 매출금액 형식으로 보고싶을때)
--   이데이터는 PIVOT 형태로 변환을 해야 한다.
...
SELECT io_dname, io_inout, io_pty, io_price, io_amt FROM tbl_iolist
WHERE (io_seq > 0 AND io_seq < 10) OR (io_seq >= 300 AND io_seq <= 310)
ORDER BY io_dname;

-- PIVOT으로 데이터 펼쳐보기
-- 매입 매출 단가 가 단가(io_price) 칼럼에 같이 저장되어 있다.
-- 이 값은 io_inout 칼럼의 값을 모르면 매입인지 매출인지 구별이 어렵다.
-- 따라서 매번 io_inout 칼럼의 값을 표시해야만 매입인지 매출인지 구별이 된다
-- 그러한 불편을 막기 위해 io_input 칼럼의 값을 조건으로 하는 DECODE 함수를 사용하여
-- 단가 부분을 매입단가, 매출단가로 분리하여 보여주도록 Query를 작성하였다.

-- 이제 io_inout 칼럼의 값을 몰라도 현재 데이터가 매입데이터인지, 매출데이터인지 알기 쉽게 되었다.
SELECT io_dname, io_inout, io_pty, 

        -- 매입단가(보기위한 가상칼럼) 칼럼의 데이터를 보여준데
        -- 만약 io_inout 칼럼에 값이 '매입' 이면, io_price 칼럼의 값을 표시하고
        -- 그렇지 않으면 0을 표시하라
...
-- 1. "거래처별로" 거래내역이 여러번 반복이루어 졌으므로 거래처 명이 여러번 나타날 것이다.
...
-- 00 회사에서 2019년 1년동안 총 매입, 총매출, 총 이익이 얼마인지 알고싶다
