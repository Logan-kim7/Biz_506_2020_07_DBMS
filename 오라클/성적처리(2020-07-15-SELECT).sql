...
-- Recored의 제한(전체 데이터 개수 > 조건에 맞는 데이터만)
-- WHERE 절에 조건을 설정하여 데이터를 조회
-- PK , UNIQUE로 설정된 칼럼을 단일 조건으로 SELETION할때는 
...
-- PK, UNIQUE로 설정된 칼럼을 복합 조건절(조건절+조건절)로 SELECTION할때는
--  레코드가 1개만 나타날 지라도 이 데이터는 리스트로 취급한다.
SELECT * FROM tbl_student
WHERE st_num >= '20001' AND st_num <= '20010';


--st_name 에 저장된 값이 독고예준 인 리스트만 보여라
--PK, UNIQUE로 설정된 칼럼이 아닌경우
-- SELECTION 을 수행했을때 나타나는 레코드가 1개 일지라도
--          이데이터는 리스트로 취급한다.
SELECT * FROM tbl_student
WHERE st_name = '독고예준';

-- PROJECTION(프로젝션)
-- SELECTION된 레코드의 개수와 무관하게, 
-- 특정 칼럼만 리스트에 포함하는 조회
...
FROM tbl_student
WHERE st_num >= '20001' AND st_num = '20010';
-- '20001' <= st_num >= '20010' <<Python 에서는 지원을하지막 아직 JAVA나 DBMS에서는 지원을 하지 않는다.



-- BETWEEN 연산자를 이용하여
-- 범위를 제한하는 SELECTION 구현하기
-- PK 키를 사용하지 않는 경우 시간차이가 조금난다 요즘은 크게 상관 없다.

SELECT st_num, st_name, st_dept, st_grade
FROM tbl_student
WHERE st_num BETWEEN '20001' AND '2010';
...
-- SELECTION 을 수행할때는 IN 연산자를 사용하여 구현할수 있다.
SELECT st_num, st_name, st_dept, st_grade
FROM tbl_student
