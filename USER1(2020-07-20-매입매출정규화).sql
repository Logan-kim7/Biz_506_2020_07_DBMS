...
-- 2개 이상의 레코드를 UPDATE를 수행하는 것은 데이터의 무결성을 해칠수 있는 수행이 된다.
...
-- 이때는 상품명 칼럼을 GROUP BY 하여 중복되지 않은 상품명 리스트만 추출한다.
SELECT io_pname, 
...
상품정보 테이블과 JOIN할수있도록 설정해야 한다.

현재 매입매출 테이블에는 상품이름이 들어있고
상품정보 테이블에는 상품코드, 상품이름, 매입 매출단가 가 들어 있다.

매입매출 테이블의 상품이름이 해당하는 상품코드를 매입매출 테이블에 UPDATE 하고
매입매출 테이블의 상품이름 칼럼을 제거한후 JOIN을 수행해야 한다.
*/

-- 1. 매입매출 테이블의 상품명과 상품정보 테이블의 상품명을 JOIN해서 
--    매입매출 테이블의 상품명이 모두 있는지 확인 하기
...
-- 이제 상품데이터가 이상없음을 알았으니 매입매출 테이블에 상품코드를 저장할 "칼럼을 추가" 
-- ALTER TABLE : 테이블의 구조(칼럼 추가, 삭제)구조변경, 칼럼의 타입변경 등을 수행하는 명령
...
--ALTER TABLE 을 할때
-- 이미 많은 데이터가 INSERT되어 잇는 상태에서 칼럼을 추가하면
-- 추가하는 칼럼은 당연히 초기값이 NULL 된다.
...
-- 상품정보에서 매입매출장 각 레코드의 상품명가 일치하는 상품코드를 찾아서
-- 매입매출장의 상품코드(io_pcode)  칼럼에 저장해주기 (업데이트 하기)

--  UPATE 명령이 Sub Query를 만나면 
--1. Sub Query에서 현재 io_pname 칼럼의 값을 유구하고 있다.
...
--      이때 Sub Qury는 반드시 1개의 칼럼 , 1개의 VO만 추출해야 한다.
...
-- iolist에 저장된 dname, dceo 칼럼으로 거래처 정보에 데이터를 조회하고
...
-- 지금생성한 tbl_buyer 테이블에서는 거래처명은 같고 대표자가 다른 데이터가 있다.
-- 이 데이터에서 거래처명으로 죄회를 하면 출력되는 레코드(row)가 2개 이상인 경우가 발생한다.
-- 따라서 이 쿼리를 실행하면 ORA-0147: single-row subquery returns more than one row 오류가 발생한다.
...
SELECT io.bcode, io_dname, b_code, b_dname
FROM tbl_iolist IO
    LEFT JOIN tbl_buyer B
...
-- tbl_iolist 에서 io_pname, io_dname, io_dceo 칼럼은 필요가 없으므로 제거한다.
...
SELECT io_sep, io_data,
        io_dcode, b_name, b_ceo,b_tel,
        io_pcode, p_name, p_iprice, p_oprice,
        io_inout,
       DECODE(io.inout,'매입', io_price,0) AS 매입단가,
       DECODE(io.inout,'매입', io_amt,0) AS 매입단가,
       DECODE(io.inout,'매출', io_price,0) AS 매출단가,
       DECODE(io.inout,'매출', io_amt,0) AS 매출단가
FROM tbl_iolist IO
     LEFT JOIN tbl_product P
        ON io.io_pcode = p.p_code
     LEFT JOIN tbl_buyer B
        ON io.io_bcode = b.b_code
ORDER BY io_bcode = b.b_code
...
WHERE io_data BETWEEN '2019-01-01' AND '2019-01-31'
ORDER BY io_data;